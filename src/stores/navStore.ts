import { defineStore } from 'pinia';
import { type BurgerAnimation } from '../types/navbar';
import { type Navbar } from '../types/navbar'

export const useNavStore = defineStore('nav', {
  state: () => ({
    burgerMenu: [
      {id: 1, unActive: 'lg:top-5 sm:top-8 top-6', active: 'top-10 origin-center rotate-45'},
      {id: 2, unActive: 'lg:top-7 sm:top-10 top-8', active: 'top-10 opacity-0'},
      {id: 3, unActive: 'lg:top-9 sm:top-12 top-10', active: 'top-10 origin-center -rotate-45'}
    ] as BurgerAnimation[],

    bigMenu: [
      {id: 1, title: 'Shop', title_2: 'Home'},
      {id: 2, title: 'Contact', title_2: 'Contact'},
      {id: 3, title: 'Sign in', title_2: 'Sign in'},
      {id: 4, title: 'Cart', title_2: 'Cart'}
    ] as Navbar[],

    dropdownMenu: [
      {id: 1, title: 'Sign in', title_2: 'Sign in'},
      {id: 2, title: 'Shop', title_2: 'Home'},
      {id: 3, title: 'Servise', title_2: 'Servise'},
      {id: 4, title: 'Contact', title_2: 'Contact'},
      {id: 5, title: 'About us', title_2: 'About us'}
    ] as Navbar[],

    docs: [
      {id: 1, title: 'Shipping & returns', title_2: 'Shipping & returns'},
      {id: 2, title: "Terms & conditions", title_2: 'Shipping & returns'},
      {id: 3, title: 'Privacy policy', title_2: 'Shipping & returns'}
    ] as Navbar[],

    socialMedia: [
      {
        id:1,
        title: '<svg xmlns="http://www.w3.org/2000/svg" class="xl:w-7 xl:h-7 lg:w-5 lg:h-5 md:w-7 md:h-7 sm:w-10 sm:h-10 w-7 h-7" viewBox="0 0 16 16"><path fill="currentColor" d="M8 0C5.829 0 5.556.01 4.703.048C3.85.088 3.269.222 2.76.42a3.917 3.917 0 0 0-1.417.923A3.927 3.927 0 0 0 .42 2.76C.222 3.268.087 3.85.048 4.7C.01 5.555 0 5.827 0 8.001c0 2.172.01 2.444.048 3.297c.04.852.174 1.433.372 1.942c.205.526.478.972.923 1.417c.444.445.89.719 1.416.923c.51.198 1.09.333 1.942.372C5.555 15.99 5.827 16 8 16s2.444-.01 3.298-.048c.851-.04 1.434-.174 1.943-.372a3.916 3.916 0 0 0 1.416-.923c.445-.445.718-.891.923-1.417c.197-.509.332-1.09.372-1.942C15.99 10.445 16 10.173 16 8s-.01-2.445-.048-3.299c-.04-.851-.175-1.433-.372-1.941a3.926 3.926 0 0 0-.923-1.417A3.911 3.911 0 0 0 13.24.42c-.51-.198-1.092-.333-1.943-.372C10.443.01 10.172 0 7.998 0h.003zm-.717 1.442h.718c2.136 0 2.389.007 3.232.046c.78.035 1.204.166 1.486.275c.373.145.64.319.92.599c.28.28.453.546.598.92c.11.281.24.705.275 1.485c.039.843.047 1.096.047 3.231s-.008 2.389-.047 3.232c-.035.78-.166 1.203-.275 1.485a2.47 2.47 0 0 1-.599.919c-.28.28-.546.453-.92.598c-.28.11-.704.24-1.485.276c-.843.038-1.096.047-3.232.047s-2.39-.009-3.233-.047c-.78-.036-1.203-.166-1.485-.276a2.478 2.478 0 0 1-.92-.598a2.48 2.48 0 0 1-.6-.92c-.109-.281-.24-.705-.275-1.485c-.038-.843-.046-1.096-.046-3.233c0-2.136.008-2.388.046-3.231c.036-.78.166-1.204.276-1.486c.145-.373.319-.64.599-.92c.28-.28.546-.453.92-.598c.282-.11.705-.24 1.485-.276c.738-.034 1.024-.044 2.515-.045v.002zm4.988 1.328a.96.96 0 1 0 0 1.92a.96.96 0 0 0 0-1.92zm-4.27 1.122a4.109 4.109 0 1 0 0 8.217a4.109 4.109 0 0 0 0-8.217zm0 1.441a2.667 2.667 0 1 1 0 5.334a2.667 2.667 0 0 1 0-5.334z"/></svg>',
        title_2: ''
      },
      {
        id:2,
        title: '<svg xmlns="http://www.w3.org/2000/svg" class="xl:w-7 xl:h-7 lg:w-5 lg:h-5 md:w-7 md:h-7 sm:w-10 sm:h-10 w-7 h-7" viewBox="0 0 15 15"><path fill="none" stroke="currentColor" d="m4.5 13.5l3-7m-3.236 3a2.989 2.989 0 0 1-.764-2V7A3.5 3.5 0 0 1 7 3.5h1A3.5 3.5 0 0 1 11.5 7v.5a3 3 0 0 1-3 3a2.081 2.081 0 0 1-1.974-1.423L6.5 9m1 5.5a7 7 0 1 1 0-14a7 7 0 0 1 0 14Z"/></svg>',
        title_2: ''
      },
      {
        id: 3,
        title: '<svg xmlns="http://www.w3.org/2000/svg" class="xl:w-7 xl:h-7 lg:w-5 lg:h-5 md:w-7 md:h-7 sm:w-10 sm:h-10 w-7 h-7" viewBox="0 0 24 24"><path fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 10v4h3v7h4v-7h3l1-4h-4V8a1 1 0 0 1 1-1h3V3h-3a5 5 0 0 0-5 5v2H7"/></svg>',
        title_2: ''
      },
      {
        id: 4,
        title: '<svg xmlns="http://www.w3.org/2000/svg" class="xl:w-7 xl:h-7 lg:w-5 lg:h-5 md:w-7 md:h-7 sm:w-10 sm:h-10 w-7 h-7" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" d="M15.021 3.343c.509-.087 1.078-.116 1.614-.025a4.85 4.85 0 0 1 2.54 1.273c.456.01.905-.08 1.302-.208a5.36 5.36 0 0 0 1.098-.501l.009-.006a.75.75 0 0 1 1.042 1.037c-.207.315-.496.877-.819 1.507l-.155.301c-.185.36-.375.724-.552 1.036c-.111.196-.23.395-.35.567v.274A12.34 12.34 0 0 1 8.287 21.03a12.32 12.32 0 0 1-6.694-1.97a.75.75 0 0 1 .5-1.374a7.471 7.471 0 0 0 4.033-.642a4.858 4.858 0 0 1-2.61-2.922a.75.75 0 0 1 .147-.722l.01-.01A4.848 4.848 0 0 1 2.05 9.793v-.052a.75.75 0 0 1 .553-.724A4.84 4.84 0 0 1 2.09 6.84a4.9 4.9 0 0 1 .65-2.442a.75.75 0 0 1 1.232-.1a10.89 10.89 0 0 0 7.006 3.93a4.85 4.85 0 0 1 2.562-4.406c.402-.214.934-.385 1.482-.479ZM3.743 10.891a3.35 3.35 0 0 0 2.503 2.164a.75.75 0 0 1 .072 1.453c-.272.083-.551.14-.834.173a3.358 3.358 0 0 0 2.59 1.3a.75.75 0 0 1 .45 1.339a8.97 8.97 0 0 1-3.548 1.695a10.82 10.82 0 0 0 3.313.515h.009A10.838 10.838 0 0 0 19.25 8.607v-.535a.75.75 0 0 1 .186-.495c.07-.079.19-.261.36-.56c.16-.282.338-.622.523-.981l.033-.066a4.992 4.992 0 0 1-1.593.097a.75.75 0 0 1-.47-.237a3.35 3.35 0 0 0-1.904-1.032a3.42 3.42 0 0 0-1.11.025a3.605 3.605 0 0 0-1.028.323a3.35 3.35 0 0 0-1.678 3.74a.75.75 0 0 1-.767.925a12.39 12.39 0 0 1-8.149-3.627a3.41 3.41 0 0 0-.063.657v.002a3.34 3.34 0 0 0 1.486 2.785A.75.75 0 0 1 4.64 11a4.798 4.798 0 0 1-.897-.11Z" clip-rule="evenodd"/></svg>',
        title_2: ''
      },
      {
        id: 5,
        title: '<svg xmlns="http://www.w3.org/2000/svg" class="xl:w-7 xl:h-7 lg:w-5 lg:h-5 md:w-7 md:h-7 sm:w-10 sm:h-10 w-7 h-7" viewBox="0 0 24 24"><path fill="currentColor" fill-rule="evenodd" d="M5.788 14.02a.746.746 0 0 0 .132.031a456.056 456.056 0 0 1 .844 2.002c.503 1.202 1.01 2.44 1.121 2.796c.139.438.285.736.445.94c.083.104.178.196.29.266a.88.88 0 0 0 .186.088c.32.12.612.07.795.009a1.313 1.313 0 0 0 .304-.15L9.91 20l2.826-1.762l3.265 2.502c.048.037.1.068.156.093c.392.17.772.23 1.13.182c.356-.05.639-.199.85-.368a1.992 1.992 0 0 0 .564-.728l.009-.022l.003-.008l.002-.004v-.002l.001-.001a.756.756 0 0 0 .04-.133l2.98-15.025a.752.752 0 0 0 .014-.146c0-.44-.166-.859-.555-1.112c-.334-.217-.705-.227-.94-.209c-.252.02-.486.082-.643.132a3.458 3.458 0 0 0-.26.094l-.011.005l-16.714 6.556l-.002.001a2.296 2.296 0 0 0-.167.069a2.522 2.522 0 0 0-.38.212c-.227.155-.75.581-.661 1.285c.07.56.454.905.689 1.071c.128.091.25.156.34.199c.04.02.126.054.163.07l.01.003l3.169 1.067Zm14.138-9.152h-.002a.785.785 0 0 1-.026.011L3.164 11.444a.818.818 0 0 1-.026.01l-.01.003a1.126 1.126 0 0 0-.09.04a.851.851 0 0 0 .086.043l3.142 1.058a.75.75 0 0 1 .16.076l10.377-6.075l.01-.005a1.59 1.59 0 0 1 .124-.068c.072-.037.187-.091.317-.131c.09-.028.357-.107.645-.014a.854.854 0 0 1 .588.689a.84.84 0 0 1 .003.424c-.07.275-.262.489-.438.653c-.15.14-2.095 2.016-4.014 3.868l-2.613 2.52l-.465.45l5.872 4.502a.536.536 0 0 0 .251.04a.229.229 0 0 0 .117-.052a.495.495 0 0 0 .103-.12l.002-.001l2.89-14.573a1.858 1.858 0 0 0-.267.086h-.002Zm-8.461 12.394l-1.172-.898l-.284 1.805l1.456-.907Zm-2.247-2.68l1.165-1.125l2.613-2.522l.973-.938l-6.52 3.817l.035.082a339.2 339.2 0 0 1 1.22 2.92l.283-1.8a.747.747 0 0 1 .231-.435Z" clip-rule="evenodd"/></svg>',
        title_2: ''
      }
    ] as Navbar[]
  })
});